...
<div class="container-fluid py-4 px-md-4">
    ...
    <div class="mb-4 pb-3 border-bottom">
        <div class="d-flex align-items-center gap-2 mb-1">
            ...
            <span class="text-muted small font-monospace">CHỈNH SỬA</span>
        </div>
        <div class="d-flex flex-wrap justify-content-between align-items-center">
            <div>
                <h2 class="h3 fw-bold mb-1" style="font-family: 'Space Grotesk', sans-serif; letter-spacing: -0.02em;">
                    <i class="bi bi-pencil-square me-2" style="color: var(--bellroy-orange);"></i>Chỉnh Sửa: {{ $event->name }}
                </h2>
                <p class="text-muted small mb-0">
                    Cập nhật thông tin hiển thị và danh sách các mẫu laptop thuộc sự kiện này.
                </p>
            </div>
    ...
    <form action="{{ route('admin.events.update', $event) }}" method="POST" enctype="multipart/form-data" id="eventEditForm">
        @csrf
        @method('PUT')
        <div class="row g-4">
            <!-- Left Column: Event Info & Banner Settings -->
            <div class="col-lg-5">
                    ...
                    <div class="card-body">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" id="name" name="name" 
                                   ...
                                   placeholder="Ví dụ: Siêu Sale Mùa Thu, Top Laptops...">
                            <div class="form-text small text-muted">
                                Tên xuất hiện trên thanh Tab ngoài Cửa Hàng.
                            </div>
                        </div>
                        ...
                        <div class="mb-3 p-3 rounded-2" style="background: var(--surface-muted); border: 1px solid var(--border-color);">
                            ...
                            <div class="row g-2">
                                <div class="col-4">
                                    <label class="display-type-option d-block border rounded-2 p-2 text-center bg-white cursor-pointer transition-all {{ old('display_type', $event->display_type ?? 'slider') === 'slider' ? 'border-primary shadow-sm active-type' : '' }}" style="cursor: pointer;">
                                        <input type="radio" name="display_type" value="slider" class="d-none" {{ old('display_type', $event->display_type ?? 'slider') === 'slider' ? 'checked' : '' }}>
                                ...
                                <div class="col-4">
                                    <label class="display-type-option d-block border rounded-2 p-2 text-center bg-white cursor-pointer transition-all {{ old('display_type', $event->display_type) === 'banner' ? 'border-danger shadow-sm active-type' : '' }}" style="cursor: pointer;">
                                        <input type="radio" name="display_type" value="banner" class="d-none" {{ old('display_type', $event->display_type) === 'banner' ? 'checked' : '' }}>
                                ...
                                <div class="col-4">
                                    <label class="display-type-option d-block border rounded-2 p-2 text-center bg-white cursor-pointer transition-all {{ old('display_type', $event->display_type) === 'both' ? 'border-success shadow-sm active-type' : '' }}" style="cursor: pointer;">
                                        <input type="radio" name="display_type" value="both" class="d-none" {{ old('display_type', $event->display_type) === 'both' ? 'checked' : '' }}>
                        ...
                        <div class="mb-3 p-3 rounded-2 border" id="bannerConfigCard" style="background: rgba(205, 76, 32, 0.03); border-color: rgba(205, 76, 32, 0.2) !important;">
                            <label class="form-label fw-bold small text-dark d-flex align-items-center gap-1" for="banner_image">
                                <i class="bi bi-card-image" style="color: var(--bellroy-orange);"></i> ẢNH BANNER SỰ KIỆN
                            </label>

                            <!-- Existing Banner Preview (if available) -->
                            @if($event->banner_image)
                                <div class="mb-2 p-2 border rounded bg-dark" id="currentBannerBox">
                                    <div class="small text-white-50 mb-1 font-monospace" style="font-size: 10px;">ẢNH BANNER HIỆN TẠI:</div>
                                    <img src="{{ $event->banner_url }}" alt="{{ $event->name }}" class="w-100 rounded" style="max-height: 120px; object-fit: cover;">
                                    <div class="form-check form-switch mt-2 ps-5">
                                        <input class="form-check-input ms-0" type="checkbox" role="switch" id="remove_banner" name="remove_banner" value="1">
                                        <label class="form-check-label text-warning small fw-bold ms-2" for="remove_banner">
                                            <i class="bi bi-trash me-1"></i>Xóa bỏ ảnh banner này
                                        </label>
                                    </div>
                                </div>
                            @endif
                            ...
                            <div class="form-text small text-muted">
                                {{ $event->banner_image ? 'Chọn tệp mới nếu bạn muốn thay thế banner cũ.' : 'Khuyến nghị 1920x600px (16:9 hoặc 21:9). Tối đa 10MB.' }}
                            </div>
                            ...
                            <div id="bannerPreviewBox" class="mt-2 d-none">
                                <div class="position-relative border rounded-2 overflow-hidden shadow-sm" style="max-height: 140px; background: #000;">
                                    ...
                                    <button type="button" id="btnRemoveBannerPreview" class="btn btn-sm btn-dark position-absolute top-0 end-0 m-1 py-0 px-1 opacity-75" title="Hủy ảnh mới">
                            ...
                            <div class="mt-3">
                                ...
                                <input type="text" class="form-control form-control-sm font-monospace" id="banner_link" name="banner_link" 
                                       value="{{ old('banner_link', $event->banner_link) }}" 
                                       placeholder="Ví dụ: /products?category=gaming hoặc https://...">
                                <div class="form-text small text-muted">
                                    Khách bấm vào banner sẽ được mở liên kết này.
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" id="headline" name="headline" 
                                   ...
                                   placeholder="Ví dụ: Laptop Mới Đón Đầu Xu Hướng 2026">
                            <div class="form-text small text-muted">
                                Dòng tiêu đề cỡ lớn phía trên slider khi khách bấm chọn tab này.
                            </div>
                        </div>
                        ...
                        <div class="mb-3">
                            <div class="form-check form-switch p-0 ps-5">
                                <input class="form-check-input ms-0" type="checkbox" role="switch" id="is_active" name="is_active" value="1" {{ old('is_active', $event->is_active) ? 'checked' : '' }}>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control" id="description" name="description" rows="2" 
                                      placeholder="Mô tả tóm tắt ý nghĩa sự kiện...">{{ old('description', $event->description) }}</textarea>
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <div class="card-header bg-transparent py-3 d-flex flex-wrap justify-content-between align-items-center gap-2">
                        <div>
                            <h5 class="card-title mb-0 fw-bold d-flex align-items-center gap-2" style="font-size: 0.95rem;">
                                ...
                                2. SẢN PHẨM THUỘC SỰ KIỆN (<span id="selectedCount">{{ count($selectedProductIds) }}</span> ĐÃ CHỌN)
                            </h5>
                    ...
                    <div class="card-body p-3">
                        ...
                        <div class="border rounded-2 p-2 bg-light bg-opacity-25" style="max-height: 480px; overflow-y: auto;" id="productPickContainer">
                            @forelse($products as $product)
                                @php
                                    $isSelected = in_array($product->id, old('product_ids', $selectedProductIds));
                                @endphp
                                <label class="product-item-row d-flex align-items-center p-2 rounded-2 mb-1 border bg-white cursor-pointer" 
                                       style="cursor: pointer; transition: all 0.2s ease; {{ $isSelected ? 'background: rgba(205, 76, 32, 0.06); border-color: var(--bellroy-orange);' : '' }}"
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Handle remove existing banner checkbox
    const removeBannerCb = document.getElementById('remove_banner');
    const currentBannerBox = document.getElementById('currentBannerBox');
    removeBannerCb?.addEventListener('change', function() {
        if (this.checked) {
            currentBannerBox.style.opacity = '0.35';
            currentBannerBox.style.filter = 'grayscale(100%)';
        } else {
            currentBannerBox.style.opacity = '1';
            currentBannerBox.style.filter = 'none';
        }
    });
});
</script>
